...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 animate-fade-in-up">
    <div>
        ...
        <p class="text-[#777] font-medium mt-1 text-sm">Denah interaktif status ketersediaan meja secara real-time.</p>
    </div>
    <div class="flex items-center gap-4 bg-white p-3 rounded-2xl shadow-sm border border-[#E5E5E5] self-start sm:self-auto">
        <div class="flex items-center gap-2">
            <div class="w-4 h-4 rounded-full bg-[#FF4B4B] shadow-inner"></div>
...
<div class="bg-[#f7f7f7]/50 p-6 md:p-10 rounded-[3rem] shadow-inner border border-[#E5E5E5]/50 relative overflow-hidden animate-fade-in-up">
    <!-- Decorative Elements -->
    <div class="absolute top-0 left-1/2 transform -translate-x-1/2 w-64 h-8 bg-gray-200 rounded-b-3xl shadow-sm flex items-center justify-center">
        <span class="text-[10px] font-extrabold uppercase tracking-widest text-[#AFAFAF]">Pintu Masuk</span>
    </div>
    <div class="absolute bottom-10 right-10 w-32 h-32 bg-[#58CC02]/10 rounded-3xl border-4 border-white shadow-sm flex items-center justify-center rotate-12">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mt-8 relative z-10">
        <?php for($i = 1; $i <= $totalTables; $i++): ?>
            ...
            <div class="relative group cursor-pointer transform hover:-translate-y-2 transition-all duration-300">
                <div class="<?= $bgColor ?> w-full aspect-square rounded-2xl flex flex-col items-center justify-center shadow-none <?= $shadowColor ?> relative overflow-hidden border-4 border-white">
                    ...
                    <span class="text-white font-extrabold text-2xl relative z-10 drop-shadow-md"><?= $i ?></span>
                </div>
