...
<body class="p-8 max-w-4xl mx-auto">
    ...
    <div class="mb-8 no-print flex justify-between items-center bg-[#f7f7f7] p-4 rounded-xl">
        <a href="<?= base_url('admin/riwayat') ?>" class="text-gray-600 font-bold hover:underline">← Kembali</a>
        <button onclick="window.print()" class="bg-[#4BB200] text-white px-6 py-2 rounded-lg font-bold hover:bg-[#4BB200] shadow-md">
            🖨️ Cetak / Simpan PDF
        </button>
    </div>
    ...
    <table class="w-full text-left border-collapse mb-8 print-border">
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td class="p-3 border border-black text-sm text-right font-bold text-[#4B4B4B]">Rp <?= number_format($r['total_bayar'], 0, ',', '.') ?></td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        </tbody>
        <tfoot>
            <tr class="bg-gray-200 print-border">
                ...
                <th class="p-3 border border-black text-right text-lg font-extrabold">Rp <?= number_format($omzet, 0, ',', '.') ?></th>
            </tr>
        </tfoot>
    </table>
