...
<div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 animate-fade-in-up">
    <div>
        ...
        <p class="text-gray-500 font-medium mt-1 text-sm">Konfirmasi pembayaran sebelum pesanan diteruskan ke koki dapur.</p>
    </div>
    <!-- Auto-refresh Indicator -->
    <div class="bg-white px-4 py-2 rounded-full border border-gray-200 shadow-sm flex items-center gap-2 self-start sm:self-auto">
...
<?php if(empty($pesanan)): ?>
    <div class="bg-white rounded-[2rem] p-12 md:p-16 text-center border border-gray-100 shadow-sm mt-6 flex flex-col items-center justify-center animate-fade-in-up">
        <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-5 shadow-inner">
            <span class="text-4xl">💤</span>
        </div>
        <h2 class="text-xl md:text-2xl font-black text-gray-800 mb-2">Kasir Sedang Kosong</h2>
        <p class="text-gray-500 font-medium text-sm">Belum ada antrean pembayaran dari pelanggan saat ini.</p>
    </div>
<?php else: ?>
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-7">
        <?php foreach($pesanan as $p): ?>
            <div class="bg-white rounded-[2rem] shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-t-[6px] border-blue-500 overflow-hidden flex flex-col group animate-fade-in-up">
                <div class="p-5 border-b border-gray-50 relative">
                    ...
                    <div class="flex justify-between items-start mb-3 relative z-10">
                        ...
                        <span class="bg-blue-50 text-blue-600 text-xs font-black uppercase px-3 py-1.5 rounded-xl border border-blue-100 shadow-sm flex-shrink-0">
                            Meja <?= esc($p['no_meja']) ?>
                        </span>
                    </div>
                    
                    <div class="bg-gradient-to-br from-gray-50 to-slate-100 p-4 rounded-2xl border border-gray-100/50 mt-3 relative z-10 shadow-inner">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Tagihan</p>
                        <p class="text-2xl md:text-3xl font-black text-green-600 tracking-tighter drop-shadow-sm">Rp <?= number_format($p['total_bayar'], 0, ',', '.') ?></p>
