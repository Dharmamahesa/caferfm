...
<head>
    ...
    <style>
        ...
        /* Menyembunyikan elemen yang tidak sesuai filter */
        .hidden-item { display: none !important; }
    </style>
</head>
<body class="bg-gray-50 pb-24"> <div class="bg-orange-600 text-white p-5 rounded-b-3xl shadow-md sticky top-0 z-40">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">Kafe Gamified</h1>
                <p class="text-orange-100 text-sm mt-1">Selamat datang! Silakan pesan menu.</p>
            </div>
    ...
    <div class="flex gap-3 p-4 mt-2 overflow-x-auto no-scrollbar">
        <button onclick="filterMenu('semua', this)" class="filter-btn active-filter bg-gray-800 text-white px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all">Semua Menu</button>
        <button onclick="filterMenu('makanan', this)" class="filter-btn bg-gray-200 text-gray-700 px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all">Makanan</button>
        <button onclick="filterMenu('minuman', this)" class="filter-btn bg-gray-200 text-gray-700 px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all">Minuman</button>
        <button onclick="filterMenu('snack', this)" class="filter-btn bg-gray-200 text-gray-700 px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all">Snack</button>
    </div>

    <div class="p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <?php 
        // Menggabungkan semua array kategori untuk di-looping
            ...
            <?php foreach($semua_menu as $item): ?>
                <div class="menu-card bg-white rounded-2xl shadow-sm border border-gray-100 p-3 flex flex-col" data-kategori="<?= esc($item['kategori']) ?>">
                    <div class="w-full h-32 bg-gray-200 rounded-xl mb-3 flex items-center justify-center text-gray-400 text-xs overflow-hidden">
                        <?php if($item['foto']): ?>
                            <img src="<?= base_url('uploads/menu/' . $item['foto']) ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                            <span>No Image</span>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        // Inisialisasi state keranjang dari LocalStorage (jika ada) atau array kosong
        ...
        function updateCartUI() {
            ...
            // Munculkan floating cart dari bawah jika ada isinya
            if (totalQty > 0) {
        ...
        function goToCart() {
            // Arahkan ke halaman keranjang/checkout
            window.location.href = "<?= base_url('keranjang') ?>";
        }

        // Jalankan saat halaman pertama kali dimuat untuk mengecek keranjang sebelumnya
